...
import sys
sys.path.append(dir_path + '/neural_network/')
...
if(config.robot == 'go2'):
    m = mujoco.MjModel.from_xml_path(dir_path + '/../../simulation/robot_model/unitree_go2/scene.xml')
    #m = mujoco.MjModel.from_xml_path(dir_path + '/robot_model/unitree_go2/scene_rough.xml')
elif(config.robot == 'aliengo'):
    m = mujoco.MjModel.from_xml_path(dir_path + '/../../simulation/robot_model/aliengo/scene.xml')
    #m = mujoco.MjModel.from_xml_path(dir_path + '/robot_model/aliengo/scene_rough.xml')
elif(config.robot == 'hyqreal'):
    m = mujoco.MjModel.from_xml_path(dir_path + '/../../simulation/robot_model/hyqreal/scene.xml')
else:
...
time_to_wait_after_last_reset = 1000
force_reset = False
model_nn = SFC()
...
with mujoco.viewer.launch_passive(m, d, show_left_ui=False, show_right_ui=False) as viewer:
    ...
    while i < maximum_step:
        ...
        if((i % training_step == 0.0) and i > time_to_wait_after_last_reset):
        ...
        #reset if the robot fall
        if(d.qpos[2] < 0.2 or 
           np.abs(rpy_angles[0]) > 0.7 or
           np.abs(rpy_angles[1]) > 0.7 or force_reset):
            mujoco.mj_resetDataKeyframe(m, d, 0)
            time_until_last_reset = 0
            force_reset = False
            i = i + 1
            ...
            time_until_last_reset = time_until_last_reset + 1
        
        state_old = state_current.copy()
        state_current = {
            'position': com_pos,
        ...
        reference_old = reference_state.copy()
        ...
        # Solve OCP ---------------------------------------------------------------------------------------    
        mpc_frequency = config.simulation_params['mpc_frequency'] + np.random.uniform(-20, 20)
        if(i % round(1/(mpc_frequency*simulation_dt)) == 0): 
            ...
            status = controller.compute_control(state_current, reference_state, contact_sequence,
                                                external_wrenches=disturbance_wrench_predicted)
