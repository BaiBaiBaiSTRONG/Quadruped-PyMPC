...
class Centroidal_Model_Robust:
    def __init__(self,) -> None: 
        ...
        self.stance_proximity_GRF = cs.SX.sym("stanceProximity_GRF", 4, 1)
        ...
        param = cs.vertcat(self.stance_param, self.mu_friction, self.stance_proximity,
                           self.base_position, self.base_yaw, self.external_wrench, self.stance_proximity_GRF)
    ...
    def forward_dynamics(self, states: np.ndarray, inputs: np.ndarray, param: np.ndarray) -> cs.SX:
        ...
        # Disabling tangential force near a touchdown event
        stance_proximity_GRF_FL = param[19]
        stance_proximity_GRF_FR = param[20]
        stance_proximity_GRF_RL = param[21]
        stance_proximity_GRF_RR = param[22]

        foot_force_fl[0] = foot_force_fl[0] * (1-stance_proximity_GRF_FL)
        foot_force_fl[1] = foot_force_fl[1] * (1-stance_proximity_GRF_FL)

        foot_force_fr[0] = foot_force_fr[0] * (1-stance_proximity_GRF_FR)
        foot_force_fr[1] = foot_force_fr[1] * (1-stance_proximity_GRF_FR)

        foot_force_rl[0] = foot_force_rl[0] * (1-stance_proximity_GRF_RL)
        foot_force_rl[1] = foot_force_rl[1] * (1-stance_proximity_GRF_RL)

        foot_force_rr[0] = foot_force_rr[0] * (1-stance_proximity_GRF_RR)
        foot_force_rr[1] = foot_force_rr[1] * (1-stance_proximity_GRF_RR)
    ...
    def export_robot_model(self,) -> AcadosModel:
        ...
        self.param = cs.vertcat(self.stance_param, self.mu_friction, self.stance_proximity, self.base_position, 
                           self.base_yaw, self.external_wrench, self.stance_proximity_GRF)
