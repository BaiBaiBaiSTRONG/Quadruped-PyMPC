...
from centroidal_model_robust import Centroidal_Model_Robust
import numpy as np
import scipy.linalg
import casadi as cs
import copy
...
class Acados_NMPC_Robust:
    def __init__(self):
        ...
        if(self.use_stability_constraints == False and self.use_zero_order_robust_optimization):
            print("You cannot use zero order robust optimization without stability constraints!")
        ...
        self.centroidal_model = Centroidal_Model_Robust()
    ...
    def create_ocp_solver_description(self, acados_model) -> AcadosOcp:
        ...
        if(self.use_zero_order_robust_optimization):
            # zoro stuff
            self.fdbk_K_mat = np.zeros((nu, nx))
            self.unc_jac_G_mat = np.eye(nx)
            self.W_mat = np.zeros((nx, nx))

            # initial uncertainty
            self.P0_mat = np.zeros((nx, nx))
                
            self.backoff_scaling_gamma = 1.0
            self.backoff_eps: float=1e-8
    ...
    def tighten_constraints(self, state_acados, contact_sequence):
        ...
        max_reduction = config.mpc_params['max_constraint_tightening']

        stability_margin_reduction_FL_FR = 0
        stability_margin_reduction_FR_RR = 0
        stability_margin_reduction_RR_RL = 0
        stability_margin_reduction_RL_FL = 0
        stability_margin_reduction_FL_RR = 0
        stability_margin_reduction_FR_RL = 0


        for j in range(self.horizon):
            
            # Propagate the uncertainty with the linearized model matrices
            temp_P_mat = copy.deepcopy(self.P0_mat)
            temp_A = self.acados_ocp_solver.get_from_qp_in(j, "A")
            temp_B = self.acados_ocp_solver.get_from_qp_in(j, "B")
            if(j < config.mpc_params['state_covariance_propagation_num_step']):
                if(j < config.mpc_params['process_noise_propagation_num_step']):
                    temp_AK = temp_A - temp_B @ self.fdbk_K_mat
                    self.P0_mat = temp_AK @ temp_P_mat @ temp_AK.T + self.W_mat
                else:
                    temp_AK = temp_A - temp_B @ self.fdbk_K_mat
                    self.P0_mat = temp_AK @ temp_P_mat @ temp_AK.T 

            #self.P0_mat = np.diag(self.P0_mat)
        
            # Compute the restriction and dont tighten the constraints 
            # at last node and at the first.
            if j < self.horizon and j > 0:
                # we should make the derivative....
                if(j < config.mpc_params['constraint_tightening_num_step']):

                    #temp = cs.vertcat(state_acados, np.zeros((24,)),
                    #    FL_contact_sequence[j], FR_contact_sequence[j], 
                    #    RL_contact_sequence[j], RR_contact_sequence[j])
                    
                    state_j = copy.deepcopy(self.acados_ocp_solver.get(j, "x"))
                    input_j = copy.deepcopy(self.acados_ocp_solver.get(j, "u"))
                    temp = cs.vertcat(state_j, input_j,
                                        FL_contact_sequence[j], FR_contact_sequence[j], 
                                        RL_contact_sequence[j], RR_contact_sequence[j])


                    # derivative of the constraint
                    if(FL_contact_sequence[j] == 1 and FR_contact_sequence[j] == 1):
                        h_FL_FR = np.array(self.constraint_FL_FR_jac_fun(temp)[0:12])
                        stability_margin_reduction_FL_FR = (self.backoff_scaling_gamma * np.sqrt(h_FL_FR@self.P0_mat[0:12,0:12]@h_FL_FR.T + self.backoff_eps))[0][0]
                        stability_margin_reduction_FL_FR = np.where(np.abs(stability_margin_reduction_FL_FR) > max_reduction, np.sign(stability_margin_reduction_FL_FR)*max_reduction, stability_margin_reduction_FL_FR)

                    
                    if(FR_contact_sequence[j] == 1 and RR_contact_sequence[j] == 1):
                        h_FR_RR = np.array(self.constraint_FR_RR_jac_fun(temp)[0:12])
                        stability_margin_reduction_FR_RR = (self.backoff_scaling_gamma * np.sqrt(h_FR_RR@self.P0_mat[0:12,0:12]@h_FR_RR.T + self.backoff_eps))[0][0]
                        stability_margin_reduction_FR_RR = np.where(np.abs(stability_margin_reduction_FR_RR) > max_reduction, np.sign(stability_margin_reduction_FR_RR)*max_reduction, stability_margin_reduction_FR_RR)


                    if(RR_contact_sequence[j] == 1 and RL_contact_sequence[j] == 1):
                        h_RR_RL = np.array(self.constraint_RR_RL_jac_fun(temp)[0:12])
                        stability_margin_reduction_RR_RL = (self.backoff_scaling_gamma * np.sqrt(h_RR_RL@self.P0_mat[0:12,0:12]@h_RR_RL.T + self.backoff_eps))[0][0]
                        stability_margin_reduction_RR_RL = np.where(np.abs(stability_margin_reduction_RR_RL) > max_reduction, np.sign(stability_margin_reduction_RR_RL)*max_reduction, stability_margin_reduction_RR_RL)

                    
                    if(RL_contact_sequence[j] == 1 and FL_contact_sequence[j] == 1):
                        h_RL_FL = np.array(self.constraint_RL_FL_jac_fun(temp)[0:12])
                        stability_margin_reduction_RL_FL = (self.backoff_scaling_gamma * np.sqrt(h_RL_FL@self.P0_mat[0:12,0:12]@h_RL_FL.T + self.backoff_eps))[0][0]
                        stability_margin_reduction_RL_FL = np.where(np.abs(stability_margin_reduction_RL_FL) > max_reduction, np.sign(stability_margin_reduction_RL_FL)*max_reduction, stability_margin_reduction_RL_FL)

                    if(FL_contact_sequence[j] == 1 and RR_contact_sequence[j] == 1):
                        h_FL_RR = np.array(self.constraint_FL_RR_jac_fun(temp)[0:12])
                        stability_margin_reduction_FL_RR = (self.backoff_scaling_gamma * np.sqrt(h_FL_RR@self.P0_mat[0:12,0:12]@h_FL_RR.T + self.backoff_eps))[0][0]
                        stability_margin_reduction_FL_RR = np.where(np.abs(stability_margin_reduction_FL_RR) > max_reduction, np.sign(stability_margin_reduction_FL_RR)*max_reduction, stability_margin_reduction_FL_RR)

                    if(FR_contact_sequence[j] == 1 and RL_contact_sequence[j] == 1):
                        h_FR_RL = np.array(self.constraint_FR_RL_jac_fun(temp)[0:12])
                        stability_margin_reduction_FR_RL = (self.backoff_scaling_gamma * np.sqrt(h_FR_RL@self.P0_mat[0:12,0:12]@h_FR_RL.T + self.backoff_eps))[0][0]
                        stability_margin_reduction_FR_RL = np.where(np.abs(stability_margin_reduction_FR_RL) > max_reduction, np.sign(stability_margin_reduction_FR_RL)*max_reduction, stability_margin_reduction_FR_RL)
                    

                # Tighten the constraints: the upper bound is reduced and the lower bound is increased
                # by the same amount. The original bound are computed in the previous function (set_stage_constraint)
                self.upper_bound[j][self.nsh_stability_start] -= stability_margin_reduction_FL_FR
                self.lower_bound[j][self.nsh_stability_start] += stability_margin_reduction_FL_FR

                self.upper_bound[j][self.nsh_stability_start+1] -= stability_margin_reduction_FR_RR
                self.lower_bound[j][self.nsh_stability_start+1] += stability_margin_reduction_FR_RR

                self.upper_bound[j][self.nsh_stability_start+2] -= stability_margin_reduction_RR_RL
                self.lower_bound[j][self.nsh_stability_start+2] += stability_margin_reduction_RR_RL

                self.upper_bound[j][self.nsh_stability_start+3] -= stability_margin_reduction_RL_FL
                self.lower_bound[j][self.nsh_stability_start+3] += stability_margin_reduction_RL_FL

                self.upper_bound[j][self.nsh_stability_start+4] -= stability_margin_reduction_FL_RR
                self.lower_bound[j][self.nsh_stability_start+4] += stability_margin_reduction_FL_RR

                self.upper_bound[j][self.nsh_stability_start+5] -= stability_margin_reduction_FR_RL
                self.lower_bound[j][self.nsh_stability_start+5] += stability_margin_reduction_FR_RL
                
                # Inform acados about the new bounds
                self.acados_ocp_solver.constraints_set(j, "uh", self.upper_bound[j])
    ...
    def compute_control(self, state, reference, contact_sequence, constraint = None, 
        ...
        stance_proximity_GRF_RR = np.zeros((self.horizon, ))

        
        tangential_force_disable_num_step = config.mpc_params['tangential_force_disable_num_step']
        # we need to disable from outside the GRF after a touchdown with some sort of clock
        FL_dt_after_touchdown = dt_after_touchdown[0]
        FR_dt_after_touchdown = dt_after_touchdown[1]
        RL_dt_after_touchdown = dt_after_touchdown[2]
        RR_dt_after_touchdown = dt_after_touchdown[3]

        if(FL_dt_after_touchdown < config.mpc_params['dt']*tangential_force_disable_num_step):
            stance_proximity_GRF_FL[0] = 1
            for d in range(0, tangential_force_disable_num_step):
                if(d+1) < self.horizon:
                    stance_proximity_GRF_FL[d+1] = 1
        if(FR_dt_after_touchdown < config.mpc_params['dt']*tangential_force_disable_num_step):
            stance_proximity_GRF_FR[0] = 1
            for d in range(0, tangential_force_disable_num_step):
                if(d+1) < self.horizon:
                    stance_proximity_GRF_FR[d+1] = 1
        if(RL_dt_after_touchdown < config.mpc_params['dt']*tangential_force_disable_num_step):
            stance_proximity_GRF_RL[0] = 1
            for d in range(0, tangential_force_disable_num_step):
                if(d+1) < self.horizon:
                    stance_proximity_GRF_RL[d+1] = 1
        if(RR_dt_after_touchdown < config.mpc_params['dt']*tangential_force_disable_num_step):
            stance_proximity_GRF_RR[0] = 1
            for d in range(0, tangential_force_disable_num_step):
                if(d+1) < self.horizon:
                    stance_proximity_GRF_RR[d+1] = 1

        if(tangential_force_disable_num_step == 0):
            stance_proximity_GRF_FL = stance_proximity_GRF_FL*0
            stance_proximity_GRF_FR = stance_proximity_GRF_FR*0
            stance_proximity_GRF_RL = stance_proximity_GRF_RL*0
        ...
        if self.use_RTI:
            ...
            for zoro_iter in range(config.mpc_params['num_qp_iterations']):
                if(zoro_iter != 0):
                    # Preparation phase
                    self.acados_ocp_solver.options_set('rti_phase', 1)
                    self.acados_ocp_solver.solve()  
                    print("preparation phase time: ", self.acados_ocp_solver.get_stats('time_tot'))

                    self.acados_ocp_solver.set(0, "lbx", state_acados)
                    self.acados_ocp_solver.set(0, "ubx", state_acados)


                time_start_tightening = time.time()
                # Costraint tightening and set stage constraint 
                self.tighten_constraints(state_acados, contact_sequence)
                print("tightening time: ", time.time() - time_start_tightening)
