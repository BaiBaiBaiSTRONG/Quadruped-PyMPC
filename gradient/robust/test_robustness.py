...
sys.path.append(dir_path + '/../gradient/robust/')
...
elif(config.mpc_params['type'] == 'robust'):
    from centroidal_nmpc_robust import Acados_NMPC_Robust
    controller = Acados_NMPC_Robust()
...
dt_after_touchdown = np.array([0, 0, 0, 0]) + 1.
...
init_com_pos = copy.deepcopy(d.qpos[0:3])
init_angle = copy.deepcopy(d.qpos[3:7])
...
with mujoco.viewer.launch_passive(m, d, show_left_ui=False, show_right_ui=False) as viewer:
    ...
    while True:
        ...
        d.qpos[0:3] = init_com_pos
        d.qpos[3:7] = init_angle
        com_pos = d.qpos[0:3]
        ...
        reference_foot_FL[1] += -0.02
        reference_foot_FL[2] = 0.10
        ...
        if(i % round(1/(mpc_frequency*simulation_dt)) == 0): 
            ...
            # If we use Gradient-Based MPC
                ...
                time_start = time.time()
                if(config.mpc_params['type'] == "robust"):
                    nmpc_GRFs, \
                    nmpc_footholds, \
                    nmpc_predicted_state, \
                    status = controller.compute_control(state_current, reference_state, contact_sequence, dt_after_touchdown=dt_after_touchdown)
                else:
                    nmpc_GRFs, \
                    nmpc_footholds, \
                    nmpc_predicted_state, \
                ...
                if(controller.use_RTI or config.mpc_params['use_zero_order_robust_optimization']):
                    # preparation phase
        ...
        dt_after_touchdown[0] += simulation_dt
        dt_after_touchdown[1] += simulation_dt
        dt_after_touchdown[2] += simulation_dt
        dt_after_touchdown[3] += simulation_dt

        if(current_contact[0] == 1 and previous_contact[0] == 0):
            dt_after_touchdown[0] = 0
        if(current_contact[1] == 1 and previous_contact[1] == 0):
            dt_after_touchdown[1] = 0
        if(current_contact[2] == 1 and previous_contact[2] == 0):
            dt_after_touchdown[2] = 0
        if(current_contact[3] == 1 and previous_contact[3] == 0):
            dt_after_touchdown[3] = 0
